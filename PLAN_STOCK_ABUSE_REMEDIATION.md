...
## Этап B. TTL-автоотмена неоплаченных заказов - 1 день

- [ ] Добавить TTL-настройку.
  - Где: `config/settings.py`.
  - Новая переменная: `ORDER_PENDING_TTL_HOURS` (например, `24`).

- [ ] Реализовать задачу автоотмены.
  - Где: `orders/tasks.py` (новая Celery task), при необходимости `orders/services.py`.
  - Логика: найти заказы старше TTL, которые еще можно отменять, и отменять через `OrderCancellationService.cancel_order(...)`.
  - Не менять остатки напрямую в task, только через доменный сервис.

- [ ] Запустить по расписанию (beat/cron).
